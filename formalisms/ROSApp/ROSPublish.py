"""
__ROSPublish.py_____________________________________________________
...
class ROSPublish(ASGNode, ATOM3Type):

   def __init__(self, parent = None):
      ASGNode.__init__(self)
      ATOM3Type.__init__(self)
      self.graphClass_ = graph_ROSPublish
      ...
      self.directEditing = []
   def clone(self):
      cloneObject = ROSPublish( self.parent )
   ...
   def postCondition (self, actionID, * params):
      if actionID == self.CONNECT:
         res = self.publishOnce(params)
         if res: return res
      if self.graphObject_:
         return self.graphObject_.postCondition(actionID, params)
      else: return None
   def publishOnce(self, params):
      if len(self.in_connections_) == 0: return
